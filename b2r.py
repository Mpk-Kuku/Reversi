...
def tree(move1):		
	...
	obj1 = []
	ptr=struc()
	leg_moves=all_legal_moves(board,turn_counter)
	for i in range (6):
		for j in range (6):
			
			if(leg_moves[i][j]==True):

				countref+=1
				board1 = [[board[a][b] for a in range(6)] for b in range(6)]


				findflips(board1,i,j,turn_counter)
				for i1 in range (6):
					for j1 in range (6):
						if(board1[i1][j1]==1):
							countw+=1
				...
				obj1.append(ptr)
	max=obj1[0].heuristic_value
	mv_to_be_played=obj1[0].move
	for i in range (1,countref-1):
            if(obj1[i].heuristic_value>max):
                max=obj1[i].heuristic_value
                mv_to_be_played=obj1[i].move
	findflips(board,mv_to_be_played[1],mv_to_be_played[0],turn_counter)
    ...
    while True:
        for event in pygame.event.get():#quit
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                posx = posy = 0;chk = False;chk1 = False
                if x in range(100,340) and y in range(100,340):
                    ...
                    for j in range(6):
                        if 100+40*(j)<= y and 100+40*(j+1)>=y:
                            posy = j;chk1 = True
                            break
                if( legal_move(board,posx,posy,turn_counter) and chk and chk1):
                    print([posx,posy])
